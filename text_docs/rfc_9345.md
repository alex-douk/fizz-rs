...
4.  Delegated Credentials
   ...
   this application.  The Credential has the following structure:
      ...
      struct {
        uint32 valid\_time;
        SignatureScheme dc\_cert\_verify\_algorithm;
        opaque ASN1\_subjectPublicKeyInfo<1..2^24-1>;
      } Credential;

   valid\_time:  Time, in seconds relative to the delegation
   ...
   dc\_cert\_verify\_algorithm:  The signature algorithm of the Credential
      ...
      delegated credentials: rsa\_pss\_rsae\_sha256, rsa\_pss\_rsae\_sha384,
      and rsa\_pss\_rsae\_sha512.

   ASN1\_subjectPublicKeyInfo:  The Credential's public key, a DER-
...
4.1.1.  Server Authentication
   ...
   SHALL send a "delegated\_credential" extension in its ClientHello.
   The body of the extension consists of a SignatureSchemeList (defined
   in \[RFC8446\]):

      struct {
        SignatureScheme supported\_signature\_algorithms<2..2^16-2>;
      } SignatureSchemeList;
   ...
   handshake with an "unexpected\_message" alert.
   ...
   an "illegal\_parameter" alert.  If the server sends multiple delegated
   credentials extensions in a single CertificateEntry, the client MUST
   abort the handshake with an "illegal\_parameter" alert.

   The algorithm field MUST be of a type advertised by the client in the
   "signature\_algorithms" extension of the ClientHello message, and the
   dc\_cert\_verify\_algorithm field MUST be of a type advertised by the
   ...
   credentials MUST terminate the connection with an "illegal\_parameter"
   alert.

4.1.2.  Client Authentication

   A server that supports this specification SHALL send a
   "delegated\_credential" extension in the CertificateRequest message
   ...
   use delegated credentials sent as extensions to any other
   certificate, and SHOULD ignore them, but MAY abort the handshake with
   an "illegal\_parameter" alert.  If the client sends multiple delegated
   ...
   "signature\_algorithms" extension of the CertificateRequest message,
   and the dc\_cert\_verify\_algorithm field MUST be of a type advertised
...
4.1.3.  Validating a Delegated Credential
   ...
   DelegatedCredential.cred.valid\_time:
   ...
   2.  Verify that the delegated credential's remaining validity period
       ...
       expiry time is less than the certificate's expiry\_time.

   3.  Verify that dc\_cert\_verify\_algorithm matches the scheme indicated
...
6.  IANA Considerations
   ...
   This document registers the "delegated\_credential" extension in the
   "TLS ExtensionType Values" registry.  The "delegated\_credential"
...
7.6.  The Impact of Signature Forgery Attacks
   ...
   (if the default valid\_time is used).
   ...
   the attacker can choose any value for the valid\_time field.  Repeated
